...
MoreJSEvents.enchantmentTableChanged((event) => {
    ...
    slotList.forEach(slot => {//遍历三个槽位，对三个附魔选项进行修改
        ...
        let enchantmentPoints = {//使用附魔点数计算额外附魔数量与等级,写成object方便函数修改
            count: 100 + slot.requiredLevel * 1 - slot.enchantmentCount * 10
        }
        addEnchantment(slot, level, position, enchantmentPoints)
        slot.updateClue()
    })
})
...
let addEnchantment = (slot, level, position, enchantmentPoints) => {
    //初始化允许附魔的列表
    ...
    if (allowEnchantmentList.length !== 0)//若允许附魔列表不为空则不进行额外附魔
        //每次消耗20附魔点数试图附魔
        for (; enchantmentPoints.count > 0; enchantmentPoints.count -= 20) {
            //获取本次试图的附魔
